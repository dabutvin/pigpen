...
extension PuzzleGame {
    ...
    /// The same day with a wall that closes round the pig and is still not the best the map
    /// has in it: two stars for fifteen tiles and an apple, with a piece left over and ground
    /// left to take. It is the board the verdict card has the most to fit on — *Start over*,
    /// *Go bigger* and *Share* over the way out — so it is the one to look at when that row
    /// of buttons changes.
    static func aDayHeld(_ level: PuzzleLevel) -> PuzzleGame {
        let game = PuzzleGame(level: level)
        game.build("""
            .........
            .........
    ...
    /// The same day with the best wall it has in it: twenty-two points for ten pieces, three
    /// stars and the rainbow a pen with nothing left above it keeps. It is the day the
    /// postcard is photographed on, since a card is worth looking at with everything it can
    /// carry on it — and `DailyPostcardTests` holds this wall to being that pen.
    static func aDayAtItsBest(_ level: PuzzleLevel) -> PuzzleGame {
        let game = PuzzleGame(level: level)
        game.build("""
            .........
            ...#.....
