...
/// the pieces will reach round, shut or it is no pen at all. The windfall down here is a diamond
/// growing out of the flowstone and the hazard a boulder come off the roof, and each is worth what
/// its meadow twin was — a diamond five tiles to shut in, a boulder five to shut in with and no
...
/// in for the diamond and `x` for the boulder.
///
...
/// fields; this is that idea turned the other way up — there is a diamond on every board in the
/// world, because a diamond is the only light in the Gloamdeep and a field with none would be a
...
extension PuzzleLevel {
    ...
    /// hung straight down off the belly as a rectangle hold 16 tiles and one diamond, which is 21
    /// and a fair answer. The same twelve hung under the belly as a lozenge — stepping out west and
    /// east and closing again on the diagonal — hold 22 tiles and both diamonds, which is 32.
    ///
    ...
    /// 18 tiles and the one diamond lying inside them, which is 23; the same eleven laid as a
    /// staircase of their own, mirroring the river back at itself, shut 26 tiles and both
    /// diamonds, which is 36. A diagonal wall closes two tiles a piece
    ...
    /// squared off north of the water hold 14 tiles, both diamonds and the boulder, which is 19 —
    ...
    /// pays its five — 26 and two diamonds less one boulder, or 31. The eight tiles of free bank
    /// that piece bought are worth that twice over, which is a sum a player has to do rather than
    /// see. One of those diamonds lies out past the tip as well, so the corridor is the only way
    ...
    /// eleven tiles of dry floor in every direction, two diamonds, a boulder and nineteen pieces.
    /// Every tile of this pen is bought.
    ///
    /// It is Basalt Flats and Cobble Yard and Swept Flat again, and the answer is the answer it
    /// always was: the best block nineteen pieces can square off holds 25, reaching one diamond
    /// and paying for the boulder it cannot help swallowing, and the same nineteen
    /// run round as a diamond hold 36 tiles, both diamonds and the boulder they cannot avoid
    ...
    /// Three diamonds in a reach the glowworms have got into, and not one of them on the line the
    /// tidy pen wants. The river steps in off the roof at the north-west and away south-east, and
    /// the best block eleven pieces can hang under it holds 15 tiles and not one diamond — there is
    ...
    /// diamond above the river's arm is on the far side of the water: a pen that wants it has to
    ...
    /// which is 21 tiles and that one diamond, or 26.
    ...
    /// and hold 18 tiles and ONE diamond, which is 23 — the other is pegged out north of the bend
    ...
    /// its way — hold 34 tiles and both diamonds, which is 44. Foundry Corner asked this on a
    ...
    /// tiles, both diamonds and two of the three boulders, which comes back to 44 — the third is
    ...
    /// within a column of the east wall, three diamonds, one boulder and nineteen pieces.
    ///
    ...
    /// game at 58 tiles.
    /// Nineteen pieces squared off down the west of it hold 42 tiles with a diamond and the
    /// boulder cancelling each other out, which is 42. The same nineteen laid along the river
    /// and closed round
    /// the south as a staircase hold 58 tiles, two diamonds and that same boulder, which is 63.
    /// The third diamond lies out east past the tip of the water, where nothing these nineteen
    ...
    /// a pocket of 14 tiles off the south end of the river with two diamonds in it, and gives the
    /// pig 21 tiles of the wide floor north-east of the water with the third diamond and a boulder
