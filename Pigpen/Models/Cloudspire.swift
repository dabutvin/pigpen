...
/// a **rainbow** pooled where it came down on the turf, and the hazard a **storm** worrying
/// at the fields — and each is worth exactly what its meadow twin was, a rainbow five tiles
/// to shut in, a storm five to shut in with and no fencing at all, since a rainbow is
/// anchored to nothing and no post stands in a wind that never stops turning. So the same
/// solver that authored the eleven worlds below these authored them, with `a` standing in
/// for the rainbow and `x` for the storm.
///
...
/// pig's ground never touches is ground nothing can ever walk to, so a rainbow adrift on one
/// is the fen's bar flowers said in this world's grammar: in plain sight, five points, and
...
/// where the free wall is everywhere and the storms are staked exactly where the tidy
/// answers want to stand.
extension PuzzleLevel {
    /// A crown of open sky rings the top of the board — a one-rainbow islet adrift in it,
    ...
    /// everything turns on where the south line is drawn. A storm tower staked at the
    /// rim kills every tidy rectangle that wants a south wall through its column, and a
    /// third storm sits exactly on the straight row the block wants, so the block
    /// swallows it and stops at 24. The best pen takes the hollow, the field's top rows, and
    /// pockets down both sides of the dead column to swallow both rainbows: 38 tiles and the
    /// pair, less the swallowed storm, which is 43.
    ...
    /// rainbow staked on it where nothing will ever walk.
    ///
    ...
    /// both live rainbows on the way down: 20 tiles and the pair, which is 30. A storm
    ...
    /// answer is the answer it always was — with two rainbows to bend it and two storms
    /// staked exactly on the natural diamond's rim. The best pen is a long diamond bent to
    /// keep both storms out and both rainbows in, 41 tiles and the pair for 51; the
    /// block nudges out over one rainbow, must swallow a storm doing it, and comes to
    /// 27. The long way north to the far rainbow is the whole game.
    ...
    /// causeway is the only way over to the delta hung on the north and east rims. Across
    /// the last gap, a rainbow glitters on a dead strip nothing will ever walk to.
    ///
    ...
    /// delta, notching around the storm tower that kills every rectangle's deep east
    /// wall, and swallows both live rainbows: 37 tiles and the pair, which is 47.
    ...
    /// the tell for which stretch of it to take. Across the fjord, a rainbow rides a dead
    /// limb the size of a small field.
    ///
    /// The shore question, on the longest bank the heights have: storm pairs staked at
    /// the west and south pin every tidy rectangle to the middle of the field, so the block
    /// stops at 26 and reaches no rainbow at all. The best pen takes the whole bank
    /// frontage, both bumps, and runs deep down both sides of the dead notch for the pair
    /// of live rainbows: 41 tiles and the pair, which is 51.
    ...
    /// Rainbows hung wide of every tidy line — and two of them down where no line will ever
    /// reach, pooled on dead islets in the great southwest sky: as lost as the balloons the
    /// board is named for.
    ///
    /// The detour question, teased four ways: a storm pair kills every rectangle whose
    ...
    /// east wall down the rim to swallow the high rainbow, 36 tiles and the one, which is 41 —
    /// while the deep rainbow hangs one detour too far and is refused. One bowed for, one
    ...
    /// The windward corner: sky two deep along the whole north and west — a rainbow riding
    ...
    /// asked it, at full stretch: the right angle boxes a strip, swallows both storms and
    /// reaches one rainbow for 20, where one grand diagonal — ten fences corner to corner, the
    /// largest single wall on any ordinary field in the game — leans both banks at once,
    /// swallows the same two storms and takes BOTH live rainbows, holding 45 tiles for 45.
    /// At 55 it is the hardest ordinary field in the heights, and in the game the Whiskers
    /// alone ask more.
    ///
    /// The two live rainbows are set one straight out along the pig's row and one straight down
    /// her column, each with a storm between her and it, so neither is had for nothing —
    /// and the corner rainbow stays exactly where it was, riding the tile no pen will ever
    ...
    /// rainbow adrift on a dead islet mid-bank.
    ///
    /// The Open Sky stands outside the climb the way The Great Slough and The Great Floe do.
    /// Two gust-lines of storms rake the field — a triple across the west, a triple up
    ...
    /// east channel, wraps around the dead column, and swallows the far rainbow: 51 tiles —
    /// the biggest pen in the world, six over the boss's — and the one rainbow, which is 56.
    ...
    /// five rainbows, and the storm on the causeway swallowed on the way east, which is
    /// 65. At 76 it is the widest gap any board in the game leaves, past even The Wallow's
    /// 72. The rainbow adrift by the crown of the map stays where it is: up here even the
    /// eagle keeps a tease.
    ///
