...
struct LevelSignpost: View {
    ...
    private var plate: some View {
        Text(name)
            ...
            .foregroundStyle(GamePalette.post.opacity(standing == .shut ? 0.55 : 1))
            .lineLimit(1)
            .fixedSize()
            .padding(.vertical, 3)
            .padding(.horizontal, 9)
            .background(
                Capsule().fill(GamePalette.cream.opacity(standing == .shut ? 0.55 : 0.96))
            )
