...
struct TitleScreenView: View {
    ...
    /// the archive, the walkthrough and the gear can have up, and the game's own offer of a
    /// morning reminder — so that what the knock asks for goes up over the title screen alone.
    ...
    private func onward(from date: DailyDate) -> WayOnward? {
        ...
        guard progress.isTheWorldHeld else {
            return WayOnward(title: "Continue \(GameWorld.mudlarkMeadow.name)") {
        ...
        return WayOnward(title: "Continue \(game.name)") {
    ...
    private func yesterday(from date: DailyDate) -> WayOnward? {
        ...
        return WayOnward(title: "Yesterday's puzzle") {
