...
# own creams it read as poster paint — a colour out of a different tin than anything
# in the screenshot standing on it. This is the same pasture with the light off it,
# which is what lets the shot be the brightest thing in its own frame.
SKY = (250, 246, 236)
FIELD = (229, 234, 213)
MEADOW = (179, 199, 163)
...
def gradient(width: int, height: int, stops: list) -> Image.Image:
    ...
    kink where it starts and another where it stops, and the eye finds both. Each step
    is softened at either end, and the whole thing is drawn one pixel wide and then
    stretched, which is the same picture for a fraction of the work.
    """
...
def grained(canvas: Image.Image) -> Image.Image:
    ...
    the sky. A couple of levels of noise scattered over it put the step somewhere
...
def frame(input_path: Path, caption: str, output_path: Path) -> tuple:
    ...
    # A phone's corners are far rounder than a tablet's, and a phone's radius laid over
    # an iPad shot bites the clock out of the top-left corner — the status bar sits
    # nearer the edge there, with no notch to hold it in. Tall shots are phones; squarer
    # ones are tablets, and get the gentler curve the device actually has.
    ...
    # tile has somewhere the light comes from and the shot stands in the bright of it.
    ...
    # wide and vague; a phone on a table casts both at once — a dark seam where it meets
    # the ground and a soft breadth well beyond it — and having the two is what lets the
    # shot look set down rather than pasted on.
    ...
    # A hairline round the phone. Three of the five screens are cream to their own edges,
    # and on a cream ground the shot dissolved into the tile it stands on; a thread of
    # the game's own brown, barely there, gives the glass an edge to end at.
