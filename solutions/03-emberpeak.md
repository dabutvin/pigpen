...
The third world, solved the same way and drawn the same way. A coin takes the meadow's
apple and a flame its skull, so an `a` below is a coin worth five tiles to shut in, and
an `x` a flame worth five fewer that no piece will build on — exactly as before, and a `W`
...
has at least one flame staked in it, so there is nowhere a wall runs straight without
...
| 3 | Ashfall Terrace | 13 | 31 | 1 coin, 1 flame | 31 | 21 |
| 4 | Chestnut Scree | 12 | 27 | 2 coins, 1 flame | 32 | 21 |
| 5 | Sulphur Rill | 13 | 24 | — | 24 | 15 |
| 6 | Fumarole Field | 14 | 18 | 2 coins | 28 | 16 |
| 7 | Crater Pools | 12 | 29 | 1 coin | 34 | 17 |
| 8 | Smoulder Ridge | 16 | 32 | 3 coins, 1 flame | 42 | 30 |
| 9 | Wyrm Caldera | 20 | 38 | 3 coins, 2 flames | 43 | 27 |
...
does when it reaches the flame. It cannot be laid on one, so it steps in beside it and
leaves it out in the corner, which costs a tile of ground and saves the five points the
flame would have charged to be shut in. 25 tiles, and every one of them the channel can be
made to keep.
...
pulled in beside each of the two flames — hold 23. Neither flame is inside: on a board with
...
The tarn down the west is the only free wall on the terrace, and the coin is out south
where no tidy pen would ever go. The pen that wins is neither tidy nor cheap: it runs out
far enough south to take the coin in, then tapers away east in a long wedge.

The flame it swallows on the way is the point of the field. Walling round it would cut the
pen in two — it is in the middle of the ground, not out at the edge of it — so the pen takes
it in and pays the five. 31 tiles and a coin, less the flame, is 31, against a block
worth 21.
...
the ground beside it is cheap to wall, and the coins are strung out south where the
walling is dear.

A block beside the tarn holds one coin and 16 tiles of ground for 21. The tongue that
reaches down for the second gives up the corners to do it and swallows the flame on the way
— 27 tiles, two coins, one flame, 32. Five points for a coin is worth four tiles of
detour, and it is worth the flame as well.
...
The rill along the foot is the free wall, and the two flames are the field. There is no
block worth having here: every rectangle big enough to be worth the budget wants a wall over
one flame or the other, and a flame takes no fencing at all. Squaring off clear of both is
worth 15.

The pen that wins keeps out of the way of both instead. Its wall runs out on the diagonal
tight past the upper flame, and it pinches right in round the lower one — that notch three
...
Two vents either side of the pig, a coin beyond each of them, and a tarn at the head of
the field. The flames stand where a pen would want its west wall twice over, so the ground
...
coins are inside it, which is 27 against a block worth 17. The vents themselves cost
...
Tarns caught in the rock three tiles apart — the widest spacing in the game — with a flame
staked beyond them and a coin lying inside. At this spacing the pools give away almost
...
ring the pools imply holds 29 tiles and the coin, which is 34, and it leaves the flame on
...
to the shape of the tarn, the pen gathers all three coins on the way round: 32 tiles, three
coins, one flame, 42.

Both flames are treated exactly as the ground around them asks. The upper one sits in the
...
on, like a flame that walks. Only the pig's ground counts.
...
coins, less the two flames it swallows on the way, come to 43 — where squaring off well
...
[← Thornwood Thicket](02-thornwood-thicket.md) · [Index](README.md) · [Cogsworth City →](04-cogsworth-city.md)
