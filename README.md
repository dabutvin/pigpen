...
| `screenshots.yml` | PR to main | Build, wake a simulator, capture the title screen, the map, a board being built and a board with a pen closed on it in light + dark, post/update a PR comment |
...
- **Screenshots.** The PR screenshot images are committed to an orphan-ish `ci-screenshots` branch under `pr-<number>/` and hot-linked into a single PR comment that gets updated in place on each push. That branch is CI-only — never merge it. Files are named `<order>_<screen>_<light|dark>.png`, and each screen gets its own row in the comment. The app takes `-map`, `-puzzle` and `-penned` launch arguments so the world map, a board being built and a board with a pen closed on it can be captured without tapping through the title screen; none of them opens on a blank slate, since an untouched world has nothing on it to look at and an untouched field has no fencing and not a control on it lit. The penned board is there because a field still being built has nothing to say about the half of the screen that only a closed pen brings out: the wash on the ground, the tally, and the count on the release button. Each screen is shot in both appearances off one launch: the views read the colour scheme out of the environment, so flipping the simulator under a running app re-draws it, and the pair then shows the same board rather than two rolls of the dice.
