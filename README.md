...
- **The simulator is the slow part.** Not the build. A simulator that has never been booted on a fresh runner spends minutes getting to the point where it can install and run an app: booting, starting installd, building the runtime's shared cache the first time anything launches, attaching a display the first time anything is photographed. `.github/actions/simulator` starts all of that in the background before the project has even been generated, so it happens alongside the build instead of after it, and lets a stub app — five lines of C linked against UIKit and SwiftUI — be the one that pays for the cold install and launch. The steps that need the simulator then wait on the stage they need. If the warm-up falls over, the waiting step says so and the job carries on and pays the cold start itself.
