...
reduced motion. A gear in the corner opens settings, which holds the version number and one
red button: clearing all game data throws away every star and shuts the trail back to its
first level, so it asks before it does anything.
...
| `screenshots.yml` | PR to main | Build, wake a simulator, capture the title screen, the map, three boards and the settings sheet in light + dark, post/update a PR comment |
