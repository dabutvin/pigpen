...
The meadow is the one world the download holds whole. Every world past it is walked the same
way by a player who has not bought the game, only slower: **one new level every twenty-four
hours**. The first stop of the thicket is theirs the moment the meadow is held; tapping it
starts a day's clock, and the next stop up the trail stays shut under a lock with the time
left on the day counted down over it — `14h`, `40m` — until the day is up and the lock comes
off on its own. A level once opened is theirs for good, to walk back down to and take apart
...
- **A held day goes on a postcard.** Once the pen holds, the verdict card offers *Share*,
  ...
  on. `DailyPostcardTests` pins every word on the card, and holds the board to going
  without its answer unless asked.
...
safe to give a pig. The send-off ends the way the opening began — the pig already off after
the next place, a dark forest at the meadow's edge standing in for the open gate it first
came through — which is the next world, and a reason to go to it.
...
Tapping the meadow's last signpost stops for a film first. A player who has fenced eight
fields does not need teaching how to fence a ninth — they need telling the one thing this
map does differently, which is that there are two animals on it. Three shots: the promising
piece of land with a deer walking into it behind the pig, the pig and the deer looking at
each other under the line that says this was not a vacant lot, and then both of them under a
single pen outline that splits into two, so the shape of the answer — one pen or two,
whatever makes the floor plan work — is on screen before a piece is laid. It is lit flat and
bright where the films either side of it are lit at sunrise, because a briefing wants reading
rather than admiring.

#### The meadow held

When the last pen in the meadow holds, the world gets seen out. Three shots: the pig loose
and easy in the finished pen on ground washed gold, with windfall apples lying about it —
space, good views, plenty of apples; then the pig turned toward a dark stand of trees banked
up at the edge of the meadow, under the line that says by all accounts it should have been
satisfied; and then the pig away up the trail and into the trees, because it has started
checking other listings. The forest is drawn as a wall of dark rather than as somewhere you
can already see into, and the two shots that turn toward it fall into dusk where the film
...
Nothing in the game is recorded. Every noise is a few oscillators and an envelope, the way the icon is a few shapes, so a sound can be tuned by changing a figure and running the script again. `Tools/generate_sounds.py` writes one short WAV per sound into `Pigpen/Resources/Sounds/` and needs nothing beyond the standard library:
...
**Looking at it.** The checker reads words; it cannot see a button. `.github/workflows/screenshots-languages.yml` photographs five text-heavy screens — the title, the settings sheet, a boss board, the toll card and the record card — in every language the game ships in, and posts them to the pull request beside the English. It runs when a branch changes `Localizable.xcstrings`, not on every pull request, since a branch that moves a button by three points does not need more pictures on it.

What it is for is fit rather than wording. A translation is read in the catalog; what a diff cannot show is Russian running past the edge of a button, a line cut off mid-sentence, or a font that never loaded. The same hand crank the English gallery has is on this one too, for a run that never arrived.
...
The two workflows mint under different prefixes, and each sweeps only its own. TestFlight builds sign as `Pigpen CI <run id>` and retire each other, one idle certificate at a time. A release signs as `Pigpen Release <run id>`, and its certificate is retired only by the *next* release's sweep — never by the TestFlight builds that keep shipping `main` in between. That difference is load-bearing: a build sitting in App Review has its signature re-checked when the submission is resubmitted, and a revoked certificate fails that check (`ITMS-90035`), which is how this repo's first submission had to be rebuilt mid-review. With the prefixes split, merging to `main` while a release is under review is safe; the one rule left is not to cut a *new* release while the previous one is still in review. At rest the account holds one certificate per prefix — two — which is still inside Apple's cap.

Revoking a distribution certificate does not disturb builds already released on TestFlight or the App Store, because Apple re-signs those for distribution. What it does break, besides the Ad Hoc and direct-install builds this repo never produces, is a build still *in review* — see above.
...
│   ├── DailyPostcard.swift      # What a held day's card says: the verdict, the clock, the run, the day's address
...
│   ├── DailyPostcardCard.swift  # The card a held day is sent as: the wordmark, the board, the stars, the address
