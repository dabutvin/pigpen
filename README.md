...
- **Screenshots.** The PR screenshot images are committed to an orphan-ish `ci-screenshots` branch under `pr-<number>/` and hot-linked into a single PR comment that gets updated in place on each push. That branch is CI-only — never merge it. Files are named `<order>_<screen>_<light|dark>.png`, and each screen gets its own row in the comment. The app takes `-map` and `-puzzle` launch arguments so the world map and the board can be captured without tapping through the title screen; `-map` shows a world two levels in, since an untouched one has nothing on it to look at. Each screen is shot in both appearances off one launch: the views read the colour scheme out of the environment, so flipping the simulator under a running app re-draws it, and the pair then shows the same board rather than two rolls of the dice.
- **The simulator is the slow part.** Not the build. A simulator that has never been booted on a fresh runner spends five or six minutes getting to the point where it can install and run an app: booting, starting installd, building the runtime's shared cache the first time anything launches, attaching a display the first time anything is photographed. Both PR jobs used to pay all of that, in series, after their build had already finished, which is where all but a minute of a twelve-minute check went. `.github/actions/simulator` starts it in the background before the project has even been generated, and hands the cold install and launch to a stub app — five lines of C linked against UIKit and SwiftUI, never called, only loaded — so the real app arrives to a simulator that has done it all once already. Steps that need the simulator wait for the stage they need. If the warm-up falls over the waiting step says so, and the job carries on and pays the cold start itself.
- **Three cores, so the warm-up takes turns.** Booting is mostly waiting and shares the machine happily, but the first launch builds the runtime's shared cache and wants every core there is: running that next to the build made both two to four times slower and cost more than the overlap saved. So the boot runs alongside the build and the expensive part holds off until the job has nothing left to do but wait. For the same reason the builds name a generic simulator destination rather than the device — naming it makes xcodebuild ask CoreSimulator about a device that is halfway through booting, and it sat there for nearly three minutes waiting for an answer.
